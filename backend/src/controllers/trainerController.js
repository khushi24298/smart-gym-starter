...
export const getTrainerRoster = async (req, res) => {
  try {
    ...
    const roster = await Promise.all(trainers.map(async (trainer) => {
      ...
      return {
        ...
        scheduleSummary: assignedClasses.map((item) => `${item.day} ${item.time}`).slice(0, 3).join(', ') || 'No classes assigned'
      };
    }));
