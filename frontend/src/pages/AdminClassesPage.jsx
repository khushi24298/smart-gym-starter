...
export default function AdminClassesPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.title || !formData.trainerName || !formData.day || !formData.time) {
      setErrorMessage('Please fill required class fields.');
      return;
    }

    try {
      if (editingId) {
        await api.put(API_ENDPOINTS.admin.classById(editingId), formData);
        setMessage('Class updated successfully.');
      } else {
        await api.post(API_ENDPOINTS.admin.classes, formData);
    ...
    <div className="page">
      ...
      <form className="card admin-form" onSubmit={handleSubmit}>
        ...
        <div className="grid">
          ...
          <input className="input" name="day" value={formData.day} onChange={handleChange} placeholder="Date/Day" />
        ...
        <textarea className="input textarea" name="description" value={formData.description} onChange={handleChange} placeholder="Description" />
            ...
            {classes.map((item) => (
              <tr key={item._id}>
                ...
                <td className="row-actions">
                  ...
                  <button className="button danger" onClick={() => handleDelete(item._id)}>Delete</button>
                </td>
              </tr>
            ))}
            {classes.length === 0 && (
