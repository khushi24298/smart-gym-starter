...
export default function AdminPlansPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.name || !formData.price || !formData.durationInDays) {
      setErrorMessage('Name, price and duration are required.');
      return;
    }
    ...
    <div className="page">
      ...
      <form className="card admin-form" onSubmit={handleSubmit}>
        ...
        <div className="grid">
          ...
          <input className="input" name="durationInDays" type="number" value={formData.durationInDays} onChange={handleChange} placeholder="Duration in days" />
            ...
            {plans.map((plan) => (
              <tr key={plan._id}>
                ...
                <td className="row-actions">
                  ...
                  <button className="button danger" onClick={() => handleDelete(plan._id)}>Delete</button>
                </td>
              </tr>
            ))}
            {plans.length === 0 && (
